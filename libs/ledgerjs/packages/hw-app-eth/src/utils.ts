...
export const getParity = (
  ...
  const chainIdUint4 = getChainIdAsUint4(chainId);
 ...
 * @returns {Number}
 */
export const getChainIdAsUint4 = (chainId: BigNumber | number): number => {
  const chainIdBuff = Buffer.from(padHexString(new BigNumber(chainId).toString(16)), "hex");
  const chainIdUint4 = chainIdBuff.subarray(0, 4);

  return parseInt(chainIdUint4.toString("hex"), 16);
};
...
export const safeChunkTransaction = (
  ...
  let chunkSize = 0;
  if (payload.length % maxChunkSize > encodedVrsBuff.length) {
    chunkSize = maxChunkSize;
  } else {
    for (let i = 1; i <= maxChunkSize; i++) {
      if (payload.length % (maxChunkSize - i) > encodedVrsBuff.length) {
